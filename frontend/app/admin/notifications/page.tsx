...
function notifyAdminNotificationRefresh() {
...
function AdminNotificationsContent() {
  ...
  const fetchNotifications = useCallback(
    async (query: { page: number; limit: number; isRead?: boolean }) => {
      ...
      try {
        ...
        setErrorMessage(
          error instanceof ApiError
            ? error.message
            : "ไม่สามารถโหลดการแจ้งเตือนผู้ดูแลได้",
        );
      } finally {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-6 py-10 md:px-10">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-7 md:px-8">
          <div>
            ...
            <p className="mt-1 text-sm text-white/65">
              รายการแจ้งเตือนทั้งหมดของผู้ดูแลระบบ
            </p>
          </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center gap-1 border-b border-slate-100 bg-[#f8fafc] px-4 pb-0 pt-4">
          {filterTabs.map((tab) => (
            <button
              key={tab.value}
              type="button"
        ...
        <div className="p-5">
          {loading ? (
            <div className="flex flex-col items-center gap-3 py-12">
              ...
              <p className="text-sm text-slate-500">กำลังโหลดการแจ้งเตือน...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-12 text-center">
              ...
              <p className="text-sm text-slate-400">
                ระบบจะแจ้งเตือนเมื่อมีการอัปเดตคำขอหรือเหตุการณ์สำคัญ
              </p>
            </div>
          ) : (
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        {!item.isRead && (
                          <button
                            ...
                            disabled={
                              markingReadId === item.id || markAllLoading
                            }
          ...
          {!loading && items.length > 0 && (
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5">
              <p className="text-sm text-slate-500">
                หน้า{" "}
                <span className="font-semibold text-[#0e2d4c]">{page}</span> จาก{" "}
